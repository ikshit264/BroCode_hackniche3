...
contract crowdfunding {
    ...
    function getAllProjectsDetail() external view returns (ProjectMetadata[] memory allProjects) {
        ...
        // Use a helper function to create the metadata for each project
        for (uint256 i = 0; i < totalProjects; i++) {
            newList[i] = createProjectMetadata(i);  // Call helper function to create metadata
        }
    ...
    function createProjectMetadata(uint256 i) internal view returns (ProjectMetadata memory) {
    ...
    function getProjectsDetail(uint256[] memory _indexList) external view returns (ProjectMetadata[] memory projectsList) {
        ...
        for (uint256 index = 0; index < _indexList.length; index++) {
            if (_indexList[index] < projects.length) {
                ...
                newList[index] = createProjectMetadata(i);
            } else {
    ...
    function claimRefund(uint256 _index) validIndex(_index) external {
        ...
        // Fixed: Check that the refundClaimed array has this index before accessing it
        require(contributorIndex < projects[_index].refundClaimed.length, "Refund information not available");
